...
### Чек без QR (OCR Tabscanner)

Кнопка «Скан чека» в форме нового расхода шлёт фото на **`POST /api/expenses/from-receipt-ocr`** (сервер → [Tabscanner](https://docs.tabscanner.com/), заголовок `apikey`). В `.env` задайте **`TABSCANNER_API_TOKEN`**. Позиции и категории — тот же пайплайн ИИ, что при импорте по QR.
