...
export default async function RatingPage() {
  ...
  const medals = [
    <Trophy key="1" className="w-5 h-5 text-amber-500 dark:text-yellow-400" />,
    <Medal key="2" className="w-5 h-5 text-slate-400 dark:text-slate-300" />,
    <Award key="3" className="w-5 h-5 text-amber-700 dark:text-amber-600" />,
  ];
      ...
      <div className="space-y-4">
        {leaderboard.map((user, idx) => (
          <div
            key={user.id}
            className={cn(
              "bg-white dark:bg-slate-900 border rounded-xl p-4 shadow-sm dark:shadow-none",
              idx === 0 ? "border-amber-300 dark:border-yellow-500/30" : "border-slate-200 dark:border-slate-800"
            )}
          >
            ...
            {user.history.length > 0 && (
              <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                <p className="text-slate-500 dark:text-slate-500 text-xs font-medium mb-2">История начислений</p>
